...
const createTask = async (req, res) => {
  const { Project, taskname, reporter, assignee, priority, status, duedate, description, summary, story, deliveryteam, sprint, targetrelease, type, worklog } = req.body;
  const { attachment  } = req.files
  const attFile = attachement ? attachement[0] : null; // Check if attachments exist
  console.log(`attFile  ${attFile}`)
  Task.count().then(async (doc) => {
    if (doc >= 1) {
      Task.findOne({}).sort({ _id: -1 }).exec().then(async function (data) {
        try {
          console.log(`This is try block 1st line`)
          ...
          const newTask = new Task({
            ...
            type,
            attachment: attFile ? {
              data: fs.readFileSync(attFile[0].path),
              contentType: attFile[0].mimetype
            } : null,
            worklog
          })
          console.log(`newtask = ${newTask}`)
          await newTask.save()
          fs.unlinkSync(attFile.path)
